...
@pytest.mark.asyncio
async def test_session_memory_basic_flow():
    mem = SessionMemory(default_system_prompt="SYS")
    sid = "s1"
    await mem.ensure_session(sid)
    assert len(await mem.get_messages(sid)) == 1  # SystemMessage
    await mem.append_user(sid, "hi")
    await mem.append_assistant(sid, "hello")
...
@pytest.mark.asyncio
async def test_ensure_session_uses_default_system_prompt_once():
    ...
    await memory.ensure_session(session_id)
    messages_first = await memory.get_messages(session_id)
    ...
    await memory.ensure_session(session_id, system_prompt="OTHER")
    messages_second = await memory.get_messages(session_id)
...
@pytest.mark.asyncio
async def test_ensure_session_without_prompts_creates_empty_session():
...
@pytest.mark.asyncio
async def test_append_turn_creates_session_and_appends_pair():
    """append_turn should create session and append user+assistant in order."""
    memory = SessionMemory(default_system_prompt="SYS")
    session_id = "turn-session"

    await memory.append_turn(session_id, "user-message", "assistant-message")
...
@pytest.mark.asyncio
async def test_append_turn_does_not_duplicate_system_message_for_existing_session():
    ...
    await memory.ensure_session(session_id)
    await memory.append_user(session_id, "first-user")
    initial_messages = await memory.get_messages(session_id)
...
@pytest.mark.asyncio
async def test_clear_removes_all_messages_and_is_noop_for_unknown():
    """clear should drop all messages and be safe for unknown sessions."""
    memory = SessionMemory(default_system_prompt="SYS")
    session_id = "to-clear"

    # Seed some history
    await memory.append_turn(session_id, "user", "assistant")
    assert len(await memory.get_messages(session_id)) > 0

    # Clear should remove all messages
    await memory.clear(session_id)
    messages_after_clear = await memory.get_messages(session_id)
...
@pytest.mark.asyncio
async def test_session_memory_isolates_different_sessions():
    """Different session_ids must keep their histories isolated."""
    memory = SessionMemory(default_system_prompt="SYS")

    await memory.append_user("s1", "hello-1")
    await memory.append_assistant("s1", "world-1")

    await memory.append_user("s2", "hello-2")

    msgs_s1 = await memory.get_messages("s1")
    msgs_s2 = await memory.get_messages("s2")
...
@pytest.mark.asyncio
async def test_get_messages_returns_immutable_snapshot():
    """get_messages should return an immutable snapshot (tuple) per call."""
    memory = SessionMemory(default_system_prompt="SYS")
    session_id = "snapshot"

    await memory.append_user(session_id, "hi")
    first_snapshot = await memory.get_messages(session_id)
    ...
    await memory.append_assistant(session_id, "hello")
    second_snapshot = await memory.get_messages(session_id)
